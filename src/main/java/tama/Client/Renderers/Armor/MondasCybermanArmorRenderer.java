...
public class MondasCybermanArmorRenderer<T extends MondasCybermanArmorModel> {
    ...
    public ModelPart makeArmorParts(EquipmentSlot slot) {
        return new ModelPart(
                ...
                Map.of(
                        "head",
                        slot == EquipmentSlot.HEAD ? checkNonNull(model.Head) : EMPTY_PART,
                        "hat",
                        EMPTY_PART,
                        "body",
                        slot == EquipmentSlot.CHEST ? checkNonNull(model.Body) : EMPTY_PART,
                        "right_arm",
                        slot == EquipmentSlot.CHEST ? checkNonNull(model.RightArm) : EMPTY_PART,
                        "left_arm",
