...
public class BrackolinConsoleRenderer extends ConsoleRenderer<BrackolinConsoleTile, BrackolinConsoleModel<BrackolinConsoleTile>>{

    final WorldText text = new WorldText(4.75F / 16.0F, 2.4F / 16.0F)
            .minXScale(0.1F)
            .withColor(0xFFFFFF);

    public BrackolinConsoleRenderer(BlockEntityRendererProvider.Context context, BrackolinConsoleModel<BrackolinConsoleTile> model, ResourceLocation tex) {
        super(context, model, tex);
    }

    @Override
    public void renderAdditionalData(ITardisLevel tardis, BrackolinConsoleTile console, float partialTicks, PoseStack pose, MultiBufferSource source, int packedLight, int packedOverlay, IAdditionalConsoleRenderData data) {
//        pose.pushPose();
...
//        pose.popPose();
    ...
    @Override
    public void renderExtra(ITardisLevel level, BrackolinConsoleTile console, float partialTicks, PoseStack pose, MultiBufferSource source, int packedLight, int packedOverlay) {
        super.renderExtra(level, console, partialTicks, pose, source, packedLight, packedOverlay);

        pose.pushPose();
        model.root().translateAndRotate(pose);
        AnimationHelper.translateToCenter(pose, AnimationHelper.translateTo(pose, model.root(), "Console/BaseConsole/PanelBase/P5/PanelBaseSlant5/Monitor"));
