...
public class DeLoreanExteriorRenderer<T extends CarExteriorEntity, M extends HierarchicalModel<T>> extends EntityRenderer<T> {
        ...
        public void render(T pEntity, float pEntityYaw, float pPartialTick, PoseStack pose, @NotNull MultiBufferSource pBuffer, int pPackedLight) {
            pose.pushPose();
            pose.mulPose(Axis.XP.rotationDegrees(180.0F));
            pose.scale(1.3f, 1.3f, 1.3f);
            pose.mulPose(Axis.YP.rotationDegrees(pEntityYaw));
            pose.translate(0.0, 0, -1);
            DematAnimation dematAnim = ExteriorRenderer.getAnimationType(pEntity.getMatterStateHandler().getDematType());
            ...
            this.model.renderToBuffer(pose, pBuffer.getBuffer(this.model.renderType(this.getTextureLocation(pEntity))), pPackedLight, OverlayTexture.NO_OVERLAY, colors[0], colors[1], colors[2], colors[3]);
