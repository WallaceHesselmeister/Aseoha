...
public class HartnellConsoleRender extends TileEntityRenderer<HartnellConsoleTile> {
	...
	public static final HartnellModel MODEL = new HartnellModel();

	public HartnellConsoleRender(TileEntityRendererDispatcher rendererDispatcherIn) {
		super(rendererDispatcherIn);
	}

	@Override
	public void render(HartnellConsoleTile tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn) {
		matrixStackIn.pushPose();
		float scale = 0.0625F;
		matrixStackIn.scale((float) .64, (float) .64, (float) .64);
		matrixStackIn.mulPose(Vector3f.XP.rotationDegrees(180));
		matrixStackIn.mulPose(Vector3f.YP.rotationDegrees(180));
		matrixStackIn.translate(-0.8, -1.5, 0.8);
